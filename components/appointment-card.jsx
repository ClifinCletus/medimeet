...
/*here would have the appointment card and the dialog which shows up on the view details in the card
which would done to join the video call, adding doctor notes about the call/before it, 
cancel appointment, mark complete, details of the appointment etc 
would be for both the patient and doctor */

const AppointmentCard = ({ appointment, userRole, refetchAppointments }) => {
  const [open, setOpen] = useState(false); //to open or close the dialog appointment card
  const [action, setAction] = useState(null); // 'cancel', 'notes', 'video', or 'complete'(which server action to be done)
  ...
  const handleMarkCompleted = async () => {
    if (completeLoading) return; //loading of the corresponding server action
  ...
  //for the video call token handling from the data received on server action call
  useEffect(() => {
  ...
  // (if the current user is the doctor then they need information about the patient and vice versa)
  const otherParty =
    ...
    <>
      <Card className="border-emerald-900/20 hover:border-emerald-700/30 transition-all">
        <CardContent className="p-4">
          {/* renders the other party details here */}
          <div className="flex flex-col md:flex-row justify-between gap-4">
            ...
            <div className="flex flex-col gap-2 self-end md:self-start">
              ...
              {/* to set the video call session as complete, here doing to check if the user would be able to mark the session as complete */}
              <div className="flex gap-2 mt-2 flex-wrap">
                ...
                {/* for displaying the modal dialog with mark complete,cancel,add notes, appointment details etc */}
                <Button
                  size="sm"
                  variant="outline"
          ...
          <div className="space-y-4 py-4">
            ...
            {appointment.status === "SCHEDULED" && (
              <div className="space-y-2">
                ...
                {/* here decides if the call is ready to be joined  */}
                <Button
            ...
            <div className="space-y-2">
              ...
              {userRole === "DOCTOR" && action === "notes" ? (
                <div className="space-y-3">
                  ...
                  {/* buttons for cancelling and saving notes */}
                    <Button
                      type="button"
                      variant="outline"
                      size="sm"
                      onClick={() => {
                        setAction(null);
                        setNotes(appointment.notes || ""); //on cancelling notes, if there was any content in notes, make it there, else no content
                      }}
