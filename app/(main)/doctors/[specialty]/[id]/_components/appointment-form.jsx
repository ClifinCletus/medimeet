...
//shows a box to enter the optional medical concern in detail and show the details on the slot selected,date,doctor etc
const AppointmentForm = ({ doctorId, slot, onBack, onComplete }) => {
  ...
  // Handle form submission: from here, we are booking the slots and calling the server action for doing the slotbooking
  ...
  // Handle response after booking attempt: after successfull booking
  useEffect(() => {
    if (data) {
      if (data.success) {
        toast.success("Appointment booked successfully!");
        onComplete(); //to change the page
      }
    }
  }, [data]);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* to enter the medical concern to the doctor */}
    ...
    {/* buttons to confirm the booking and to go back to the slots area(if may need to change the slot) */}
      <div className="flex justify-between pt-2">
        <Button
          type="button"
          variant="outline"
          onClick={onBack}
          disabled={loading} //disabled when booking is happening
