//profile page where there would show the profile card and able to book appointments of the doctor
...
export function DoctorProfile({ doctor, availableDays }) {
  ...
  // Calculate total available slots: reducing the slots and finding the total count of slots
   ...
   ****** VVVV IMP *******
   *****/
  //to toggle(to set the booking slot card on right side to be shown or not)
  const toggleBooking = () => {
  ...
  //to handle the slot selection and add it to the state
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="border-emerald-900/20">
            <CardContent className="pt-6">
              ...
              <div className="flex flex-col items-center text-center">
                ...
                {/* this button on the left side would be used to display the second card on the right side to show the appointment slots to book it */}
                <Button
      ...
      <div className="md:col-span-2 space-y-6">
        {/* first card shows the basic slots information */}
        <Card className="border-emerald-900/20">
          <CardHeader>
          ...
          <div id="booking-section">
            <Card className="border-emerald-900/20">
              ...
              <CardContent className="space-y-6">
                {totalSlots > 0 ? (
                  <>
                    {/* Slot selection step: if no slot is currently selected, shows this to select the slot */}
                    {!selectedSlot && (
                      <SlotPicker
                        days={availableDays}
                    ...
                    {/* Appointment form step: after the slot is selected */}
                    {selectedSlot && (
                      <AppointmentForm
                        doctorId={doctor.id}
                        slot={selectedSlot}
                        onBack={() => setSelectedSlot(null)} // Reset selected slot to go back to slot selection
                        onComplete={handleBookingComplete} // Callback when appointment booking is successfully completed
                      />
                    )}
                  </>
                ) : (
